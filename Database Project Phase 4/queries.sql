...
/*2. ???no result caused by limited data??? List customers who have bought all perishable items available in the store.*/
...
/* ???no result caused by limited data??? 7. Find the employee details who has worked every day of the past week.*/
...
(
 ...
 FROM silver_customer S1, online_order OO, online_order_detail OOD
 WHERE S1.person_id=OO.person_id AND OO.order_number=OOD.order_number 
 AND OOD.product_id IN (SELECT Product_ID FROM Popular_Product)
) AS Pers;
...
WHERE E.membership_id=G.membership_id AND G.enrolled_date <= DATE_ADD(E.employment_date, INTERVAL 2 MONTH) 
...
WHERE S.person_id=P.id AND S.start_date < DATE_SUB(CURDATE(), INTERVAL 10 month);
...
SELECT B.bill_amount, S.name as store_name, S.address, S.contact
